...
class PassageScreen extends Component {
    ...
    if (jumpText.indexOf(" ") != -1) {
      ...
      if (activeBook) {
        this.setState({
          ...
          activeChapter: parseInt(splitChapter[1])
        });
        this._drawer.closeDrawer();
      }
    }
    ...
    return (
      <View style={styles.drawerWrapper}>
        <View style={styles.drawerHeader}>
          <TouchableOpacity style={styles.drawerVersion}>
  ...
  render() {
    const { verses, activeBook, activeChapter } = this.state;
    const swipeConfig = {
          ...
          >
            {verses.map((verse, i) => {
              ...
              return (
                <Text
                  key={i}
                  style={[styles.text, isTitle ? styles.title : null]}
                >
                  {!isTitle ? (
